...
if nargin<6
colorcycle={'m','c','g','b','r','y','k','b','g','r','c','m','y','k'};
end
...
for c=1:size(dat,2);
    ...
    hold on; 
    h(c)=plot(xrange,m,'color',colorcycle{c},'LineWidth',linewid,'linestyle',linez{c});
    
    d = squeeze(dat(:,c,:));
   
%    md = nanmean(d);
    fill_around_line(m,ste(d),colorcycle{c},xrange);
...
elseif ndims(dat)==2;
   m=squeeze(mean(dat));
    ...
    hold on; 
    plot(m,colorcycle{1},'LineWidth',2,'linestyle',linez{1});
