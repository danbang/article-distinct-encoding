function h = fillsteplotbue(dat,linewid,linez,condnames,xrange,colorcycle)
...
for c=1:size(dat,2);
    ...
    hold on; 
    h(c)=plot(xrange,m,'color',[051/255 153/255 255/255],'LineWidth',linewid,'linestyle',linez{c});
    
    d = squeeze(dat(:,c,:));
   
%    md = nanmean(d);
    fill_around_line(m,ste(d),[051/255 153/255 255/255],xrange);
...
elseif ndims(dat)==2;
   m=squeeze(nanmean(dat));
    ...
    hold on; 
    plot(m,'color',[051/255 153/255 255/255],'LineWidth',2,'linestyle',linez{1});
       
%    md = nanmean(d);
    fill_around_line(m,ste(dat),[051/255 153/255 255/255]);
end
